...
const validateInput = (
    ...
    if (!(req.query.filename || req.query.width || req.query.height)) {
        res.status(400)
        next(
            'Error: Your query string must include filename, width & height parameters'
        )
    } else if (isNaN(parseInt(`${req.query.width}`))) {
        res.status(400)
        next(
            'Error: Your query string is missing a proper WIDTH parameter - Please enter a numerical value'
        )
    } else if (isNaN(parseInt(`${req.query.height}`))) {
        res.status(400)
        next(
            'Error: Your query string is missing a proper HEIGHT parameter - Please enter a numerical value'
        )
        ...
        res.status(400)
        next(
            'Error: Filename is invalid. Please choose an extant file, make sure it is spelled correctly and make sure it is a STRING.'
        )
    }
}
